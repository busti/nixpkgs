  ...
  in {
    ...
    script = concatStringsSep " \\\n  " ([
      ...
      ++ lib.optionals (cfg.backend == "podman") [
        # $1 is the path to the file where the container ID is written. It needs to be passed as an argument because it is generated by systemd.
        "--cidfile=$1"
        "--cgroups=no-conmon"
        "--sdnotify=conmon"
        "-d"
    ...
    # store the cidfile where podman would normally store it
    # %t is replaced with the runtime directory root
    # %n is replaced with the unit name
    scriptArgs = if cfg.backend == "podman" then "%t/%n.ctr-id" else "";
